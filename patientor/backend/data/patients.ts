import  {Patient, Gender} from '../models/types';
import toNewPatientEntry from '../utils/patientsUtils';
import toNewDiagnosisEntry from '../utils/diagnosisUtils';
...
const patients : Patient[] = data.map( obj => {
    ...
    object.entries = obj.entries.map(entry => toNewDiagnosisEntry(entry));
    object.id = obj.id;
    return object;
});
